...
def cutting_ksklight_module():
    ...
    # Prelucrare fisier module
    fisier_module = filedialog.askopenfilename(initialdir=os.path.abspath(os.curdir),
                                                            title="Incarcati fisierul forecast")

    file_counter = 0
    file_progres = 0
    try:
        wb = load_workbook(fisier_module)
    except:
        pbar.destroy()
        pbargui.destroy()
        messagebox.showinfo("Fisier invalid", fisier_module + " extensie incompatibila!")
        return None
    ws = wb.worksheets[0]
    for row in ws['A']:
    ...
    if file_counter == 0:
        pbar.destroy()
        pbargui.destroy()
        messagebox.showerror('Eroare fisier', 'Fisierul este gol')
        return None
    lista_module = [[ws.cell(row=row.row, column=1).value, ws.cell(row=row.row, column=2).value]
                    for row in ws['A'] if row.value is not None]
    array_fire_ksk = []
    for q in range(len(array_wires_all)):
        for x in range(len(lista_module)):
            if array_wires_all[q][0] == lista_module[x][0] and array_wires_all[q][0] not in excluderecst:
                array_fire_ksk.append([array_wires_all[q][0], array_wires_all[q][1].lower(),
                                       array_wires_all[q][2], array_wires_all[q][3], array_wires_all[q][4],
                                       array_wires_all[q][5], array_wires_all[q][6], array_wires_all[q][7],
                                       array_wires_all[q][8], array_wires_all[q][9], array_wires_all[q][10]])
    for q in range(len(array_fire_ksk)):
        for x in range(len(lista_module)):
            if array_fire_ksk[q][0] == lista_module[x][0]:
                array_fire_ksk[q].append(lista_module[x][1])
    lista_cutting = [["CC", "Module ID", "Wire No.", "LTG PMD", "Color", "Cross Sec", "Conector 1", "Pin 1",
                      "Conector 2", "Pin 2", "Sonderltg", "Length", "Cantitate", "KANBAN-AG", "REAL NAME", "Ledset"]]
    for i in range(len(array_fire_ksk)):
        lista_cutting.append([array_fire_ksk[i][0] + array_fire_ksk[i][1] + array_fire_ksk[i][5] +
                              array_fire_ksk[i][6] + array_fire_ksk[i][7] + array_fire_ksk[i][8],
                              array_fire_ksk[i][0], array_fire_ksk[i][1], array_fire_ksk[i][2],
                              array_fire_ksk[i][3], array_fire_ksk[i][4], array_fire_ksk[i][5],
                              array_fire_ksk[i][6], array_fire_ksk[i][7], array_fire_ksk[i][8],
    ...
    for i in range(1, len(lista_cutting)):
        for x in range(len(control_matrix)):
            if lista_cutting[i][1] + lista_cutting[i][2] == control_matrix[x][0]:
                lista_cutting[i].extend([control_matrix[x][1], control_matrix[x][3], control_matrix[x][5]])
                break
    for i in range(len(lista_cutting)):
        try:
    ...
    for i in range(len(lista_cutting)):
        try:
            if lista_cutting[i][1] + lista_cutting[i][2] not in lista_unice and lista_cutting[i][15] != "":
                lista_unice.append([lista_cutting[i][1] + lista_cutting[i][2]])
        except IndexError:
            continue
    lista_cutting_twisturi = []
    lista_unice_twisturi = []
    ...
    for i in range(len(lista_cutting)):
        for x in range(len(lista_unice)):
            if lista_unice[x][0] == lista_cutting[i][1] + lista_cutting[i][2]:
                lista_unice[x].extend(lista_cutting[i])
    lista_cutting_unice = []
    lista_duplicate = []
    for i in range(len(lista_unice)):
        if lista_unice[i][15] not in lista_duplicate:
            lista_cutting_unice.append([lista_unice[i][14], lista_unice[i][15], lista_unice[i][16]])
            lista_duplicate.append(lista_unice[i][15])

    lista_cutting_unice[0].append("Cantitate")
    for i in range(1, len(lista_cutting_unice)):
        suma = 0
        for x in range(len(lista_unice)):
            if lista_unice[x][15] == lista_cutting_unice[i][1]:
                suma = int(lista_unice[x][13]) + suma
        lista_cutting_unice[i].append(suma)

    pbar['value'] += 2
    statuslabel["text"] = "Printing file                     "
    pbargui.update_idletasks()
    prn_excel_cutting_module(lista_cutting, lista_unice, lista_cutting_unice)
    end = time.time()
    pbar.destroy()
    pbargui.destroy()
    messagebox.showinfo('Finalizat!', str(end - start)[:6] + " secunde.")
